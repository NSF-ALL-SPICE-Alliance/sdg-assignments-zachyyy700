...
scatter_goal_3and4 <- ggplot(sdr_data, aes(x = goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "SDG Goals 3 & 4 by region",
       x = "Goal 3 Score",
       y = "Goal 4 Score",
       color = "Region")

ggplotly(scatter_goal_3and4) #for the second plot, why are the names like ([name], 1)
```
...
colnames(sdr_data)[which(colnames(sdr_data) == "country_code_iso3")] <- "iso_a3" #first column is now iso_a3 instead of country_code_iso3
```
...
class(sdr_data_world_joined) #is now sf dataframe
```
...
leaflet(sdr_data_world_joined) %>%
  addTiles() %>%
  setView(lat = 10, lng = 0, zoom = 2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = 
~colorQuantile("YlOrRd", goal_7_score)(goal_7_score), label = mytext)
```
#### <span style="color: red;">Challenge 2</span>

Can you edit the code chunk above that generates the map to:
```{r}
#I want to look at Goal 9: Industry, Innovation, and Infrastructure
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("YlGn", goal_9_score)(goal_9_score), label = mytext) #chose YlGn color palette
```
