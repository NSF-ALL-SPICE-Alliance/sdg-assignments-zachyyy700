...
rm(my_name) #noting the deletion of my_name in the environment
```
...
-   We use the `here()` function to cut out long file paths. Instead of starting at the home directory on our computer and having to point R towards the csv file we want to read in with *"Users/Victoria/data_science/SDR-2023-Landscape/SDR-2023-Data.csv"*, we can simply tell R to start within the directory of our current R project and find the SDR-2023-Data.csv in our data folder.
    ...
    sdr_data <- clean_names(sdr_data) #replace spaces with underscores
    ```
...
```{r, include = FALSE}
data_structure <- str(sdr_data) # looking at the data structure , that's a lot of stuff in the console lol
```
...
```{r}
library("dplyr") #had to load this in to run the "arrange" function in the next chunk
```
...
table(sdr_data$goal_7_dash) # data table on the amt of countries in each color category for sdg7
```
...
SDG_scores <- sdr_data %>%
  select(goal_16_score, goal_17_score) #selected goal 16 and 17

SDG_scores #testing new table, also appears in the environment. yay!
```
...
new_region #new variable with East and South Asian Countries, also in the environment
```
...
sdr_data %>%
  group_by(regions_used_for_the_sdr) %>%
  summarise(mean_goal_7_score_by_region = mean(population_using_at_least_basic_drinking_water_services_percent)) #rows are grouped by regions and averaged goal 7
```
...
sdr_data %>%
  select(country, population_with_access_to_clean_fuels_and_technology_for_cooking_percent,
         co2_emissions_from_fuel_combustion_per_total_electricity_output_mt_co2_t_wh, population_with_access_to_electricity_percent,
         renewable_energy_share_in_total_final_energy_consumption_percent) %>% #selecting five columns to reshape
  pivot_longer(cols = -country, names_to = "Indicator") #selected columns are now rows
```
