...
completely_na_countries <- missing_data_by_country$country[missing_data_by_country$pct_miss == 100] #a lot of island nations don't have enough data
```
...
sdr_data_imputed <- sdr_data_imputed %>%
  remove_rownames %>%
  column_to_rownames(var = "country") #error because this can't be run twice on the same variable, the country column are now row names
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) + 
  ...
  labs(title = "Most Important Variables in Predicting Maternal Mortality",
       x = "SDG Indicator",
       y = "Feature Importance (% Increase in Mean Squared Error)")
...
#looking at influential variables
importance_df2 <- as.data.frame(rf_u5mort$importance)
...
ggplot(importance_df2_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) + 
  geom_bar(stat = "identity", fill = "steelblue", color = "black") + 
  theme_minimal() + 
  labs(title = "Most Importance Variables in Predicting Under 5 Mortality",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
