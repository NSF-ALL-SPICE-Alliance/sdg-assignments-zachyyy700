...
```{r}
head(sdr_data) #viewing first few columns to see which to look at

ggplot(data = sdr_data, aes(x = goal_14_score, y = goal_15_score, color = goal_1_score)) + 
  geom_point()
```

Comparing goals 14 & 15 and colored by goal 1, there is a loose positive linear relationship between the health of aquatic/marine ecosystems and the health of land ecosystems. As for the color relationship, there doesn't seem to be a clear relationship, it seems pretty random. This plot aimed to investigate if reducing poverty for us humans, sacrificed the well-being of ecosystems for other species.
...
ggplot(sdr_data, aes(x = goal_3_score, y = country)) + geom_bar(stat = "identity")
```
...
ggplot(lac_sdr_data, aes(x = goal_3_score, y = country)) + geom_bar(stat = "identity")
```
...
ggplot(mena_data, aes(x = goal_3_score, y = country)) + 
  geom_bar(stat = "identity")
```
...
fill = regions_used_for_the_sdr)) + 
  geom_histogram(bins = 33, color = "black")
```
This is a histogram of goal 13, climate action which weighs actions like integrating climate policies and improving climate issue awareness. It's great to see higher numbers towards the higher score. One interesting note is the little bump of countries with a score of around 68-69%.
